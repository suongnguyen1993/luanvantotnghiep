 <section id="services">
        <div class="container">
            <div class="row">
                <div class="col-sm-3 text-center padding wow fadeIn" data-wow-duration="1000ms" data-wow-delay="300ms">
                    <div class="single-service">
                        <div class="wow scaleIn" data-wow-duration="500ms" data-wow-delay="300ms">
                            <a class='check_login'  data-href="practice/chitiet/1"><img src="<?php echo base_url(); ?>/public/user/images/home/review.png" alt=""></a>
                        </div>
                        <h2><a class='check_login'  data-href="practice/chitiet/1">Luyện Thi</a></h2>
                        
                    </div>
                </div>
                
                <div class="col-sm-3 text-center padding wow fadeIn" data-wow-duration="1000ms" data-wow-delay="600ms">
                    <div class="single-service">
                        <div class="wow scaleIn" data-wow-duration="500ms" data-wow-delay="600ms">
                            <a class='check_login' data-href="test/test"><img src="<?php echo base_url(); ?>/public/user/images/home/exam.png" alt=""></a>
                        </div>
                        <h2><a  href="test/mini_test">Làm Bài Kiểm Tra</a></h2>
                        
                    </div>
                </div>
                <div class="col-sm-3 text-center padding wow fadeIn" data-wow-duration="1000ms" data-wow-delay="900ms">
                    <div class="single-service">
                        <div class="wow scaleIn" data-wow-duration="500ms" data-wow-delay="900ms">
                            <a href="review/index/1"><img src="<?php echo base_url(); ?>/public/user/images/home/learn.png" alt=""></a>
                        </div>
                        <h2><a class='check_login'  data-href="review/index/1">Ôn Bài</a></h2>
                        
                    </div>
                </div>
                 <div class="col-sm-3 text-center padding wow fadeIn" data-wow-duration="1000ms" data-wow-delay="1200ms">
                    <div class="single-service">
                        <div class="wow scaleIn" data-wow-duration="500ms" data-wow-delay="1200ms">
                            <a class='check_login' data-href="vocabulary/voca/tudien"><img src="<?php echo base_url(); ?>/public/user/images/home/voca.png" alt=""></a>
                        </div>
                        <h2><a class='check_login' data-href="vocabulary/voca/tudien">Từ Điển Cá Nhân</a></h2>
                        
                    </div>
                </div>
            </div>
        </div>
    </section>
    <!--/#services-->

    <section id="action" class="responsive">
        <div class="vertical-center">
             <div class="container">
                <div class="row">
                    <div class="action take-tour">
                        <div class="col-sm-7 wow fadeInLeft" data-wow-duration="500ms" data-wow-delay="300ms">
                            <h1 class="title">I-TOEIC</h1>
                            <p>Website thông minh luyện thi TOEIC trực tuyến</p>
                        </div>
                        <div class="col-sm-5 text-center wow fadeInRight" data-wow-duration="500ms" data-wow-delay="300ms">
                            <div class="tour-button">
                                <a href="#" class="btn btn-common">TAKE THE TOUR WITH I-TOEIC</a>
                             </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
   </section>
    <!--/#action-->

    <section id="features">
        <div class="container">
            <div class="row">
                <div class="single-features">
                    <div class="col-sm-6 wow fadeInRight aling_top " data-wow-duration="500ms" data-wow-delay="300ms">
                        <h2><a href="test/mini_test">Các Bài Test</a></h2>
                        <P>Hãy cùng kiểm tra trình độ Toeic của bạn thông qua những bài Test của chúng tôi ngay. Với những bài Test này, bạn sẽ nắm được khả năng Toeic của mình để từ đó hãy cải thiện chúng ngày càng tốt hơn nhé. </P>
                    </div>

                       

                    <div class="col-sm-5 wow fadeInLeft" data-wow-duration="500ms" data-wow-delay="300ms">
                        <img src="<?php echo base_url(); ?>/public/user/images/home/test.gif" class="img-responsive" alt="">
                    </div>
                       <div class="col-sm-1 wow fadeInLeft" data-wow-duration="500ms" data-wow-delay="300ms">
                       <img style ="float:left;" src="<?php echo base_url(); ?>/public/user/images/home/tree.png" class="img-responsive" alt="">
                       <img style ="float:left;" src="<?php echo base_url(); ?>/public/user/images/home/tree.png" class="img-responsive" alt="">
                        </div>
                </div>
                <div class="single-features">
                 <div class="col-sm-2 wow fadeInLeft" data-wow-duration="500ms" data-wow-delay="300ms">
                        <img style ="height:105px; width:65px;float:right;" src="<?php echo base_url(); ?>/public/user/images/home/du.gif" class="img-responsive" alt="">
                        <img style ="float:right;" src="<?php echo base_url(); ?>/public/user/images/home/tree.png" class="img-responsive" alt="">
                        <img style ="float:right;" src="<?php echo base_url(); ?>/public/user/images/home/tree.png" class="img-responsive" alt="">
                    </div>
                    <div class="col-sm-5 wow fadeInRight" data-wow-duration="500ms" data-wow-delay="300ms">
                        <img src="<?php echo base_url(); ?>/public/user/images/home/prac.gif" class="img-responsive" alt="">
                    </div>
                    <div class="col-sm-4 col-sm-offset-1 align-right wow fadeInLeft aling_top" data-wow-duration="500ms" data-wow-delay="300ms">
                        <h2><a class='check_login' data-href="practice/chitiet/1">Luyện Các Kỹ Năng</a></h2>
                        <P>Website luyện thi Toeic trực tuyến mang đến cho bạn những bài tập bổ ích, giúp bạn có thể luyện riêng những phần Toeic còn yếu. Nhờ đó mà bạn sẽ dần cải thiện được trình độ Toeic của mình.</P>
                    </div>
                   
                </div>
                <div class="single-features">
                <div class="col-sm-2 wow fadeInLeft" data-wow-duration="500ms" data-wow-delay="300ms">
                        
                         <img style ="float:right;" src="<?php echo base_url(); ?>/public/user/images/home/tree.png" class="img-responsive" alt="">
                         <img style ="height:130px; width:65px;float:right;" src="<?php echo base_url(); ?>/public/user/images/home/tree.png" class="img-responsive" alt="">
                          <img style ="float:right;" src="<?php echo base_url(); ?>/public/user/images/home/tree.png" class="img-responsive" alt="">
                    </div>
                    <div class="col-sm-5 wow fadeInLeft" data-wow-duration="500ms" data-wow-delay="300ms">
                        <img src="<?php echo base_url(); ?>/public/user/images/home/review.gif" class="img-responsive" alt="">
                    </div>
                    <div class="col-sm-4 wow fadeInRight aling_top" data-wow-duration="500ms" data-wow-delay="300ms">
                        <h2><a class='check_login' data-href="review/review/index/1">Ôn Tập Bài Học</a></h2>
                        <P>Hãy cùng tham gia học tập cùng website Luyện thi Toeic trực tuyến, bạn sẽ được ôn lại những bài tập, bài học mà mình đã làm sai trong quá trình làm những bài Test hoặc trong quá trình luyện tập hằng ngày. Từ đó, bạn sẽ nắm vững các kỹ năng hơn.</P>
                    </div>
                </div>
                 <div class="single-features">
                    <div class="col-sm-5 col-sm-offset-1 align-right wow fadeInLeft aling_top" data-wow-duration="500ms" data-wow-delay="300ms">
                        <h2><a class='check_login' data-href="tu-dien-ca-nhan.html">Từ Điển Cá Nhân</a></h2>
                        <P>Website hổ trợ từ điển cho cá nhân bạn. Thông qua quá trình học tập cùng website, bạn có thể dịch nghĩa những từ khó và lưu chúng lại để có thể xem lại bất kỳ khi nào bạn muốn.</P>
                    </div>
                    <div class="col-sm-5 wow fadeInRight" data-wow-duration="500ms" data-wow-delay="300ms">
                        <img src="<?php echo base_url(); ?>/public/user/images/home/dic.gif" class="img-responsive" alt="">
                    </div>
                </div>
            </div>
        </div>
    </section>
     <!--/#features-->

    <section id="clients">
        <div class="container">
            <div class="row">
                <div class="col-sm-12">
                    <div class="clients text-center wow fadeInUp" data-wow-duration="500ms" data-wow-delay="300ms">
                        <p><img src="<?php echo base_url(); ?>/public/user/images/home/nen.gif" class="img-responsive" alt=""></p>
                        <h1 class="title">Happy Trips</h1>
                        <p>Hãy cùng tham gia chuyến tham quan một vòng I-Toeic nhé. Bạn sẽ thấy thật thú vị khi học tập cùng website của chúng tôi. </p>
                    </div>
                    
                </div>
            </div>
        </div>
     </section>
    <!--/#clients-->